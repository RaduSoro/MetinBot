...
class RuinForestFilter(HsvFilter):
    # Night mode acitve
    def __init__(self):
        self.hMin = 0
        self.sMin = 65
...
class Vision:
    ...
    def template_match_alpha(self, haystack_img, needle_path):
        ...
        # cv.imwrite("../metin_farm_bot/classifier/matches/"+str(random_no)+"HAYSTACK.jpg",haystack_img)
        # cv.imwrite("../metin_farm_bot/classifier/matches/"+str(random_no)+"NEEDLE.jpg",needle)
        result = cv.matchTemplate(haystack_img, needle, cv.TM_CCOEFF_NORMED)
        match_val, _, match_loc, _ = cv.minMaxLoc(result)
        print(match_val)
        if np.abs(match_val) < 0.25:
